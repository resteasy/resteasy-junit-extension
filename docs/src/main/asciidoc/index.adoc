...
Customize the `SeBootstrap` server configuration with `ConfigurationProvider`:
...
The extension does not use system properties directly - configuration is done via `ConfigurationProvider` implementations.

=== SeBootstrap Configuration

All `SeBootstrap.Configuration` options are available:
...
Refer to the link:{javadoc-url}/dev/resteasy/junit/extension/api/ConfigurationProvider.html[ConfigurationProvider Javadoc]
